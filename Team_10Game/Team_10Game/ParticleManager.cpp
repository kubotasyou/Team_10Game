...
const DirectX::XMFLOAT4 operator-(const DirectX::XMFLOAT4& Ihs, const DirectX::XMFLOAT4& rhs)
{
	XMFLOAT4 result;
	result.x = Ihs.x - rhs.x;
	result.y = Ihs.y - rhs.y;
	result.z = Ihs.z - rhs.z;
	result.w = Ihs.w - rhs.w;
	return result;
}
...
const DirectX::XMFLOAT4 operator/(const DirectX::XMFLOAT4& Ihs, float& f)
{
	XMFLOAT4 result;
	result.x = Ihs.x / f;
	result.y = Ihs.y / f;
...
bool ParticleManager::LoadTexture()
{
	...
	result = LoadFromWICFile(
		L"Resources/Texture/debugfont.png", WIC_FLAGS_NONE,
		&metadata, scratchImg);
	if (FAILED(result)) {
		return result;
	}
